...
import type { CodexAccountUsageRef, PersistedUsageAttempt, PersistedUsageEntry } from "../usage/log";
...
type Attribution = Pick<PersistedUsageAttempt, "provider" | "model" | "usageStatus" | "usage" | "totalTokens" | "codexAccountRef">;
...
function accountIdByRef(config: Pick<CodexAccountUsageIdentity, "codexAccounts">): Map<CodexAccountUsageRef, string> {
  const map = new Map<CodexAccountUsageRef, string>([["main", MAIN_CODEX_ACCOUNT_ID]]);
  for (const account of config.codexAccounts ?? []) {
    if (!isSelectableCodexPoolAccount(account)) continue;
    map.set(codexAccountLogLabel(account) as CodexAccountUsageRef, account.id);
  }
  return map;
}
...
function legacyProviderAccountId(
  provider: string,
  refs: ReadonlyMap<CodexAccountUsageRef, string>,
  ...
  if (CODEX_ACCOUNT_LOG_LABEL_RE.test(suffix)) return refs.get(suffix as CodexAccountUsageRef);
...
function attributedAccountId(
  ...
  if (attribution.codexAccountRef) return refs.get(attribution.codexAccountRef);
...
function entryAttributions(entry: PersistedUsageEntry): Attribution[] {
  if (entry.attempts?.length) {
    return entry.attempts.map(attempt => ({
      ...
      ...(attempt.codexAccountRef ? { codexAccountRef: attempt.codexAccountRef } : {}),
    }));
  }
  return [{
...
export function rollupCodexAccountUsage7d(
  ...
  for (const entry of entries) {
    ...
    for (const attribution of entryAttributions(entry)) {
      const accountId = attribution.codexAccountRef === undefined
