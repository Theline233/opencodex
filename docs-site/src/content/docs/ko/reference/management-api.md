...
| `POST /api/codex-auth/accounts/subscription/refresh-all` | 로그인된 account를 최대 5개씩 병렬 갱신하고 순서가 유지된 결과와 `total`, `succeeded`, `failed`를 반환합니다 | 개별 실패는 `results`에 기록되며 전체 batch를 중단하지 않습니다 |
