...
- На карточке каждого аккаунта Codex также показывается известный срок Plus/subscription. Сначала
  читается OAuth JWT; если дата отсутствует, **Обновить срок** выполняет ограниченный management-запрос.
  При ошибке сохраняется последнее значение, а нефорсированные повторы блокируются на 30 минут.
  Срок OAuth-токена и время сброса квоты не считаются сроком Plus.
- **Обновить все сроки** обновляет авторизованные аккаунты с параллелизмом не более пяти. Ошибка одного
  аккаунта не прерывает остальные; интерфейс показывает число успешных и неудачных обновлений, а карточка
  позволяет повторить запрос отдельно.
