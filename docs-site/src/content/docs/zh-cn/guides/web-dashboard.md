...
- 每个 Codex 账号卡片还会显示已知的 Plus/订阅到期时间。系统优先从 OAuth JWT 读取；缺失时，
  可以用 **刷新到期时间** 执行有边界的管理查询。查询失败会保留上次结果，并在 30 分钟内阻止非强制重试。
  OAuth 令牌到期或配额重置时间不会被当作 Plus 到期时间。
- **刷新全部到期时间** 会对所有已登录账号执行相同查询，最多同时刷新 5 个账号。单个账号失败不会
  中断其余账号，完成后会显示成功数和失败数；仍可用账号卡片上的按钮单独重试失败账号。
