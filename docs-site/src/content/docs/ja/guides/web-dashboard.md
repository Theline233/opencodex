...
- 各 Codex アカウントカードには、確認できた Plus/サブスクリプションの有効期限も表示されます。
  まず OAuth JWT を読み、見つからない場合は **有効期限を更新** で制限付きの管理 API 照会を行えます。
  失敗時は前回値を保持し、非強制の再試行を 30 分間抑制します。OAuth トークンの期限やクォータのリセット時刻を Plus の期限として扱うことはありません。
- **すべての有効期限を更新** はログイン済みアカウントを最大 5 件ずつ並行更新します。1 件の失敗で
  残りは中断されず、成功数と失敗数が表示されます。失敗したアカウントはカードから個別に再試行できます。
